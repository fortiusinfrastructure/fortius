...
import { ChevronDown, UserRound, LogOut, LayoutDashboard } from "lucide-react";
...
export function UserMenuF({ user }: { user: SessionUser }) {
  ...
  const displayName = user.name?.split(" ")[0] ?? user.email ?? "Cuenta";
    ...
    <div ref={ref} className="relative">
      ...
      >
        ...
        <ChevronDown
          ...
          strokeWidth={2}
          aria-hidden
          className={`inline ml-1 transition-transform ${open ? "rotate-180" : ""}`}
        />
        ]
      </button>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            role="menu"
            className="absolute right-0 top-[calc(100%+10px)] w-60 border border-[var(--border-default)] bg-[rgba(5,10,20,0.98)] shadow-xl"
          >
            ...
            <div className="border-t border-[var(--border-subtle)] py-1">
              <SignOutButton
                ...
                className="flex w-full items-center gap-3 px-4 py-2.5 text-[0.8rem] text-[var(--text-secondary)] hover:text-[var(--text-primary)] hover:bg-[rgba(255,255,255,0.04)] transition-colors disabled:opacity-60"
              />
            </div>
          </motion.div>
        )}
