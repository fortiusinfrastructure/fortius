...
export async function signUp(formData: FormData): Promise<AuthResult> {
    ...
    const emailHtml = `
        <div style="font-family: serif; max-width: 600px; margin: 0 auto; color: #333;">
            <h2 style="color: #16a34a;">Bienvenido a Fundación Fortius</h2>
            <p>Estimado/a ${fullName},</p>
            <p>Gracias por unirte a nuestra área privada. Para activar tu cuenta y acceder a la plataforma, por favor haz clic en el siguiente enlace:</p>
            <div style="margin: 30px 0;">
                <a href="${linkData.properties.action_link}" style="background-color: #16a34a; color: #fff; padding: 12px 24px; text-decoration: none; font-weight: bold; border-radius: 4px;">VERIFICAR MI CUENTA</a>
            </div>
    ...
    const emailResult = await sendEmail({
        to: email,
        subject: 'Verifica tu cuenta en Fundación Fortius',
        html: emailHtml,
        kind: 'contact_notification', // fallback kind
    });
    ...
    if (linkData?.properties?.action_link) {
        ...
        const html = `
            <div style="font-family: serif; max-width: 600px; margin: 0 auto; color: #333; line-height: 1.6;">
                ...
                <p>Hemos recibido una solicitud para restablecer la contraseña de tu cuenta en Fundación Fortius.</p>
                ...
                <hr style="border: none; border-top: 1px solid #eee; margin: 30px 0;" />
                <p style="font-size: 11px; color: #999;">Mensaje automático de Fundación Fortius. No respondas a este correo.</p>
            </div>
        `;
        await sendEmail({
            to: email,
            subject: 'Restablece tu contraseña — Fundación Fortius',
            html,
