...
export function ExpertClient({ member }: { member: TeamMember | ExternalExpert }) {
    ...
    // Generar datos ficticios para Highlights (en un caso real, vendrían de Supabase o CMS)
    const publications = [
        ...
        <main className="pt-[var(--nav-height)] pb-24 md:pb-36 bg-[var(--color-neutral-1000,#0a111e)] min-h-screen">
            ...
            <header className="mx-auto max-w-[var(--container-max)] px-[var(--container-px)] pt-16 md:pt-24 pb-16 border-b border-[var(--border-subtle)]">
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-20 items-start">
                    ...
                    >
                        <Bracketed variant="kicker">{isTeamMember ? "Equipo" : "Experto Vinculado"}</Bracketed>
                        <div>
                        ...
                        <div className="prose prose-invert prose-p:text-[var(--text-secondary)] prose-p:leading-relaxed max-w-none pt-4">
                            ...
                            {/* Simulamos una trayectoria extendida para el MVP */}
                            <p>
                                Con una extensa trayectoria en entornos complejos, su trabajo se ha enfocado en
                                la intersección entre políticas públicas, toma de decisiones estratégicas y
                                la construcción de alianzas institucionales. Ha asesorado a líderes del sector 
                                privado y fundaciones de ámbito internacional.
                            </p>
                        </div>
                    </motion.div>
                </div>
            ...
            {/* Trayectoria Académica (Solo para Juan Ángel Soto) */}
            {member.slug === "juan-angel-soto" && (
                <section className="mx-auto max-w-[var(--container-max)] px-[var(--container-px)] py-20 border-b border-[var(--border-subtle)]">
                    <div className="mb-12">
                        <Bracketed variant="kicker">Trayectoria Académica</Bracketed>
                    </div>
            ...
            <section className="mx-auto max-w-[var(--container-max)] px-[var(--container-px)] py-20">
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 lg:gap-24">
                    ...
                    {/* Formulario de Contacto y Archivo */}
                    <div className="space-y-16">
                        {/* CTA / Contacto */}
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease }}>
                        ...
                        <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, ease }}>
                            <Bracketed variant="tag">Archivo Personal</Bracketed>
                            <div className="mt-8 border border-[var(--border-subtle)] bg-[var(--color-neutral-900)] p-6">
                                <div className="flex items-center gap-4 text-[var(--text-secondary)] hover:text-[var(--text-primary)] cursor-pointer transition-colors group">
                                    ...
                                    <div>
                                        <h4 className="font-display text-[1.1rem]">Acceso a Biblioteca Histórica</h4>
                                        <p className="text-[0.8rem] mt-1 text-[var(--text-tertiary)]">Memorias, investigaciones pasadas y papers archivados.</p>
                                    </div>
                                </div>
                            </div>
                        </motion.div>
