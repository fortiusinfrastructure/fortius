...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: 'login-page' });
...
export default async function LoginPage({ params }: Props) {
    ...
    return (
        <main className="relative flex min-h-screen flex-col items-center justify-center px-4 py-16"
            ...
            {/* Brand lockup */}
            <Link href="/" className="mb-10 flex flex-col items-center gap-1 group" aria-label={t('back-aria')}>
            ...
            <p className="mt-8 text-center text-[10px]" style={{ color: 'var(--text-tertiary)' }}>
                {t('terms-pre')}{' '}
                <Link href="/aviso-legal" className="underline underline-offset-2 hover:opacity-80">{t('terms-link')}</Link>
                {' '}{t('terms-mid')}{' '}
                <Link href="/politica-de-privacidad" className="underline underline-offset-2 hover:opacity-80">{t('privacy-link')}</Link>.
            </p>
        </main>
    );
}
