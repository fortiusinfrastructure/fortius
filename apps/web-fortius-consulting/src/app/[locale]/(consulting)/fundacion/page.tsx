...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
    ...
    const t = await getTranslations({ locale, namespace: "fundacion-page" });
    return { title: t("meta-title"), description: t("meta-desc") };
}

export default async function FundacionPage({ params }: Props) {
    const { locale } = await params;
    const t = await getTranslations({ locale, namespace: "fundacion-page" });

    const PILLARS = [
        { title: t("pillar1-title"), description: t("pillar1-desc") },
        { title: t("pillar2-title"), description: t("pillar2-desc") },
        { title: t("pillar3-title"), description: t("pillar3-desc") },
        { title: t("pillar4-title"), description: t("pillar4-desc") },
    ];
    ...
    return (
        <main id="main-content" className="pt-[var(--nav-height)]">
            ...
            >
                <div className="mx-auto max-w-[var(--container-max)] px-[var(--container-px)]">
                    ...
                    >
                        ...
                        <span className="px-1.5">{t("kicker")}</span>
                        <span style={{ color: "var(--color-foundation-500)" }}>]</span>
                    </div>
            ...
            <section className="mx-auto max-w-[var(--container-max)] px-[var(--container-px)] py-24 md:py-32">
                <Bracketed variant="kicker">{t("pillars-kicker")}</Bracketed>
