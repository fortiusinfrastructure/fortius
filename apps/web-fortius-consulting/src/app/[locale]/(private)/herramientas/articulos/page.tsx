...
type Category = "politica" | "sociedad-civil";
...
export default function ArticulosUploadPage() {
  const [category, setCategory] = useState<Category>("politica");
  ...
  const addFiles = useCallback((incoming: FileList | File[]) => {
    const docxFiles = Array.from(incoming).filter((f) =>
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch("/api/admin/articulos/convert", {
        method: "POST",
        body: formData,
      });
      ...
      if (!res.ok) {
        ...
        setResults(
          files.map((f) => ({ filename: f.name, status: "error", error })),
        );
        return;
      }
    ...
    <main className="mx-auto max-w-2xl px-6 py-16 space-y-10">
      ...
      <div className="space-y-2">
        ...
        <p className="text-sm text-neutral-500">
          Sube uno o varios archivos <code>.docx</code> y se importarán
          directamente a la base de datos. Los nombres de archivo deben seguir
          la convención{" "}
          <code>YYYY_MM_DD. Tipo (ACCESO). Título.docx</code>.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        >
          ...
          <p className="mt-1 text-xs text-neutral-400">
            Solo archivos .docx — puedes seleccionar varios a la vez
          </p>
          <input
            ref={inputRef}
            type="file"
        ...
        {results.length > 0 && !uploading && (
          ...
          >
            {successCount > 0 && (
              <span>
                {successCount} artículo{successCount !== 1 ? "s" : ""}{" "}
                importado{successCount !== 1 ? "s" : ""} correctamente.{" "}
              </span>
            )}
            {errorCount > 0 && (
              <span>
        ...
        <button
          ...
          className="w-full rounded-lg bg-neutral-900 px-5 py-3 text-sm font-medium text-white transition-opacity disabled:cursor-not-allowed disabled:opacity-40 hover:opacity-90"
        >
          {uploading
            ? "Importando…"
            : `Importar ${files.length ? `${files.length} archivo${files.length !== 1 ? "s" : ""}` : "artículos"}`}
        </button>
      </form>
      ...
      <details className="group rounded-lg border border-neutral-200 text-sm">
        ...
        <div className="border-t border-neutral-200 px-4 py-4 text-neutral-600 space-y-3">
          <p>
            El nombre del archivo determina la fecha, el tipo y el acceso del
...
Ejemplos:
  2025_03_15. Informe (PAGO). La agenda europea.docx
  2025_04_02. Nota Inteligencia Política (ABIERTO). ¿Quién manda?.docx
  2025_04_20. Eventos & Oportunidades. Cumbres de mayo.docx`}
          </pre>
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-neutral-200 text-left text-neutral-500">
                <th className="pb-1 pr-4 font-medium">Tipo en el nombre</th>
                <th className="pb-1 font-medium">Tipo resultante</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-neutral-100">
              {[
                ...
                <tr key={kind}>
                  <td className="py-1 pr-4 text-neutral-600">{label}</td>
                  <td className="py-1 font-mono text-neutral-700">{kind}</td>
                </tr>
              ))}
            </tbody>
          </table>
